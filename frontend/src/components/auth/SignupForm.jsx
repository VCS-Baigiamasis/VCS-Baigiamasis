    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <form className="bg-white p-8 rounded-lg shadow-lg w-full md:w-full" onSubmit={handleSubmit}>
          ...
          <input type="date" max={maxDate()} onChange={(e) => setDateOfBirth(e.target.value)} value={dateOfBirth} className={`w-full px-4 py-2 border rounded-lg ${isSubmitted && dateOfBirth === '' ? 'border-red-500 bg-red-100' : 'border-gray-300'}`} required/>
          {ageError && <p className="text-red-600 mt-2">{ageError}</p>}

          <label className="block text-gray-700 mb-2">Phone Number:</label>
          <input type="tel" pattern="\+[0-9]{1,3}[0-9]{6,14}" onChange={(e) => setPhoneNumber(e.target.value)} value={phoneNumber} className={`w-full px-4 py-2 border rounded-lg ${isSubmitted && phoneNumber === '' ? 'border-red-500 bg-red-100' : 'border-gray-300'}`} required/>
